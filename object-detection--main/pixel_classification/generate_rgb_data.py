...
def read_pixels(folder, verbose = False):
  ...
  for filename in os.listdir(folder):
    ...
    # convert from BGR (opencv convention) to HSV
    ...
    # # (changes): to use mean values in the training samples rather than the first pixel.
    # tmp = img.astype(np.float64)/255
    # X[i] = tmp.mean(axis=(0, 1))
    i += 1
